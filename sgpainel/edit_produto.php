...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE produto SET titulo=%s, texto=%s, id_cat=%s, valor=%s, `data`=%s WHERE id=%s",
...
mysql_select_db($database_conexao, $conexao);
$query_categoria = "SELECT id, tipo FROM categoria ORDER BY tipo ASC";
		...
		<form action="<?php echo $editFormAction; ?>" method="POST" name="form" id="form" accept-charset="iso-8859-1">
			...
			<?php }?>
				  <table width="400" border="0" cellpadding="2" cellspacing="3" class="texto_formulario">
                    ...
                    <tr>
                      <td colspan="2"><select name="categoria" id="categoria">
                        <?php
do {  
?>
                        <option value="<?php echo $row_categoria['id']?>"<?php if (!(strcmp($row_categoria['id'], $row_produto['id_cat']))) {echo "selected=\"selected\"";} ?>><?php echo $row_categoria['tipo']?></option>
                        <?php
