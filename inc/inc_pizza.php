<?php 
global $MySQL; //Torna a variсvel acessэvel em todas as funчѕes
function MyCONN(){ //faz a conexуo com o servidor 
...
$password_conexao = "pizza121314";
 ...
 $conexao=mysql_connect ($hostname_conexao, $username_conexao, $password_conexao) or die (' Nуo Conectado' . mysql_error());
...
$sql = trim($sql);//limpa espaчos vazio   
...
$conexao=MyCONN();
$query_busca = "SELECT en.id,en.titulo,op.id as cat,op.opcao,op.n FROM opcao op
inner join enquete en on (en.id = op.id_quest)
where en.ativo = '1' order by en.id desc";
...
// enquete listar
function LISTENQUETE($codigo) {
$conexao=MyCONN();
$query_busca = "select op.id,op.opcao,op.n from enquete enq
inner join opcao op on(op.id_quest = enq.id)
where op.id = '$codigo'  ";
$venquete_list= mysql_query($query_busca, $conexao) or die(mysql_error());
return $venquete_list;
$totalRows_enquete_list = mysql_num_rows($venquete_list);
$row_enquete_list = mysql_fetch_assoc($venquete_list);
return $row_enquete_list;

}


// enquete update
function UPDATENQUETE($codigo,$total) {
$conexao=MyCONN();
$query_busca = "update opcao set
                n='$total' where id = '$codigo'
 ";
$venquete_upd= mysql_query($query_busca, $conexao) or die(mysql_error());
return $venquete_upd;
$totalRows_enquete_upd = mysql_num_rows($venquete_upd);
$row_enquete_upd = mysql_fetch_assoc($venquete_upd);
return $row_enquete_upd;

}


// adicionar promoчуo
function ADDPROMOCAO($promocao,$opcao,$id_promocao,$cliente) {
$conexao=MyCONN();
$vdata=date("y/m/d")." ".date("h:i:s");
$query_busca = "INSERT INTO promocao
(id_promocao,id_cliente,id_opcao,promocao,data)
values
('$id_promocao','$cliente','$opcao','$promocao','$vdata')
";
$arq= mysql_query($query_busca, $conexao) or die(mysql_error());
return $arq;
$totalRows_arq = mysql_num_rows($arq);
...
// se o cliente ja esta cadastrado
function LISTPROMO($cliente,$id_promocao) {
$conexao=MyCONN();
$query_busca = "select * from promocao where id_cliente = '$cliente' and id_promocao = '$id_promocao' ";
